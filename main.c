...
void cpu_execute(CPU *cpu, uint8_t opcode, uint8_t operand){
    ...
    switch (mop){
        ...
        case JMP:
            ...
            break;
        case LDARegB:
            cpu -> RegisterB = rom[operand & 0x7F];
            CPU -> PC += 2;
            break;
        case HLT:
            cpu -> Halted = true;
...
int main(void){
    ...
    uint8_t program[] = {    //PC
        ...
        LDA, 0x10,                  //0x02 load rom[0x10] t  RegisterA
        ADD, 0x00,                 //0x04 RegA = RegA+RegB
        STA_ADDR, 0x12        //0x06 Storage RegA content to sram 0x12
    ...
    while (!cpu.halted){
        ...
        printf("PC: 0x%02X, Opcode: 0x%02X, operand: 0x%02X, RegA: %d, RegB: %d, ZF: %d\n", cpu.PC, opcode, operand, cpu.RegisterA, cpu.RegisterB, cpu.zero_flag, cpu_execute(&cpu, opcode, operand));
    }
