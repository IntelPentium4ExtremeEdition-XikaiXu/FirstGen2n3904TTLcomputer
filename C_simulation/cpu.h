 ...
 */
typedef enum{
	OPCODE_NOP,      	    //000 - null processing, do nothing, used for filling, aligned, timing lentancy process - null flag - null op_  tent
	OPCODE_LDA_imm5        //001 - init ACC register, setting default value - zero flag(if imm5 == 0, z = 1) - acc <- imm5_  tent
	OPCODE_ADD_imm5,       //010 - acculmate, PC counter increment - Z/C - acc <- acc + imm5_  tent
	OPCODE_SUB_imm5,       //011 - Subtraction, comparision - Z/C - acc <- acc - imm5_  tent
	OPCODE_AND_imm5,       //100 - bit masking, testing certain bit, clear bit - Z/C - acc <- acc & imm5_  tent
	OPCODE_SHR,            //101 - reg left shifing, used for driving the lcd/dividing  - Z/C - acc <- acc >> 1_  tent
	OPCODE_JZ_imm5,        //110 - statement jump, if statement ACC = 0 detecting - null flag - if Z =1, PC <- PC + sign_extend(imm5)_  tent
	OPCODE_OUT_imm5        //111 - ACC to the mainbus, sending the data through this inst - null flag - mainbus <- acc _  tent
} cpu_opcode_t;
